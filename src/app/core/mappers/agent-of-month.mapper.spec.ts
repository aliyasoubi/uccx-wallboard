...
describe('agent-of-month.mapper', () => {
  it('cross-references the roster by agentID to resolve the display name', () => {
    ...
    const result = mapAgentsOfMonth({ agentID: 'A1002', photoUrl: '' }, agents);
  ...
  it('resolves name to null when the agent id has no roster match', () => {
    const result = mapAgentsOfMonth({ agentID: 'A9999', photoUrl: '' }, [buildAgent()]);
    expect(result.name).toBeNull();
  });

  it('normalizes an empty photoUrl to null so the UI can show a defined fallback', () => {
    const result = mapAgentsOfMonth({ agentID: 'A1002', photoUrl: '' }, [buildAgent()]);
    expect(result.photoUrl).toBeNull();
  });

  it('preserves a non-empty photoUrl', () => {
    const result = mapAgentsOfMonth({ agentID: 'A1002', photoUrl: 'https://example.com/a.jpg' }, [buildAgent()]);
